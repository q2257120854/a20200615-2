...
function sys_ShowClassByTemp($classid,$tempid,$show=0,$cline=0){
	...
	$no=1;
	$changerow=1;
	$sql=$empire->query("select classid,classname,bname,islast,sonclass,tbname,intro,classimg,infos from {$dbtbpre}enewsclass where bclassid='$classid' and showclass=0 order by myorder,classid".$limit);
	while($r=$empire->fetch($sql))
	{
		//显示类别数据数
		if($show)
