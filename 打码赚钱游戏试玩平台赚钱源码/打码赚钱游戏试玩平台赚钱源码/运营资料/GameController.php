...
class GameController extends Controller {
    ...
    function actionRewards() {
        if (Yii::app()->request->isAjaxRequest) {//是否ajax请求
            ...
            if (empty($gamedata["valid"]) && !empty($gamedata)) {
                ...
                $content1 = "试玩平台：" . $title1 . ",奖励稻米：" . $gamedata["hlb"];
                ...
                if (!empty($gameimpact)) {
                    ...
                    $content2 = "试玩平台:" . $title2 . "奖励稻米" . $gameimpact["hlb"];
    ...
    function actionWowreturn(){
        ...
        if($mypass==$wowre['sign']){
            if($wowre['immediate']==1){
                $content1 =  "体验".$wowre['offer_name']."奖" . $point . "稻米";
    ...
    function actionZcinfo() {
        try {
            ...
            $gamezm_model->gid = $_GET['gid']; //稻米田游戏id
    ...
    function actionZc345(){
        try {
            ...
            if (!empty($gamezm_num)) {
                ...
                $gamezm_model->gid = $game_info["id"]; //稻米田游戏id
    ...
    function actionPpwanreginfo() {
        try {
            ...
            $gamezm_model->gid = $_GET['adcode']; //稻米田游戏id
            $gamezm_model->cid = $game_info["cid"]; //厂商id
    ...
    function actionReginfo() {
        try {
            ...
            $gamezm_model->gid = $_GET['hlz_gid']; //稻米田游戏id
    ...
    function actionGradexdl() {
        try {
            if (Yii::app()->request->isAjaxRequest) {//是否ajax请求
                $memid = Yii::app()->request->getParam('memid'); //稻米田游戏id
                $gid = Yii::app()->request->getParam('gid'); //稻米田游戏id
    ...
    function actionGrade55e5() {
        try {
            if (Yii::app()->request->isAjaxRequest) {//是否ajax请求
                ...
                $gid = Yii::app()->request->getParam('gid'); //稻米田游戏id
                $game_info = Game::model()->findBySql("select * from {{game}} where id= " . $gid);
                $url = $game_info["return_url"] . "?&uid=" . $memid . "&sign=" . md5("374" . $game_info["game_id"] . $memid . "46ec88e4456d7cdef58e59c0fedfb0ea");
               $resu = file_get_contents($url);
                $result = json_decode($resu);
                $data = $result->user_info;
                $level = $data->level;
    ...
    function actionSelgrade() {
        try {
            if (Yii::app()->request->isAjaxRequest) {//是否ajax请求
                $guid = Yii::app()->request->getParam('guid'); //稻米田游戏id
                $gid = Yii::app()->request->getParam('gid'); //稻米田游戏id
                $game_info = Game::model()->findBySql("select * from {{game}} where id= " . $gid);
...
//                    if (!empty($memid)) {
//                        $title = "兑换凡凡元宝";
//                        $hlb = $ybhlb_model["ybnum"] * 8500; //兑换消耗的稻米
...
//                                if ($data->Status == 1) {
//                                    $reason = $title . "消耗了" . $hlb . "稻米,共兑换了：" . $ybhlb_model["ybnum"] . "个元宝";
//                                    $hlb_model = $this->updhlb(-$hlb, 24, $reason, $memid, 0); //扣除稻米
...
//                                    Yii::app()->user->setFlash('msg', '兑换失败,本次没有扣除稻米');
//                                }
//                                $ybhlb_model->update();
//                            }
//                        } else {
//                            Yii::app()->user->setFlash('msg', '稻米不足!');
//                        }
//                    } else {
...
//                    if (!empty($memid)) {
//                        $title = "兑换凡凡元宝";
//                        $hld = $ybhld_model["ybnum"] * 11000;  //兑换消耗的稻谷
...
//                                $data = json_decode($result);
//                                if ($data->Status == 1) {
//                                    $reason = $title . "消耗了" . $hld . "稻谷,共兑换了：" . $ybhld_model["ybnum"] . "个元宝";
//                                    $hld_model = $this->updhld(-$hld, 24, $reason, $memid, 0); //扣除稻米
//                                    $this->sendmessage($title, $reason, 1, $memid); //1为系统消息
...
//                            }
//                        } else {
//                            Yii::app()->user->setFlash('msg', '稻谷不足!');
//                        }
//                    } else {
//                        Yii::app()->user->setFlash('msg', '请先登录会员!');
//                    }
