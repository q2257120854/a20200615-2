        ...
        if (empty($memfirst_info["idcode"])) {
            if (isset($_POST['Memfirst'])) {
                ...
                if ($memfirst_info->save()) {
                    ...
                    $this->redirect("thirdly");   // $this->redirect("second");
                }
            }
    ...
    function actionNote() {
        ...
        if (!empty($mem)) {
            if (Yii::app()->request->isAjaxRequest) {//是否ajax请求
				...
				"password" => "123456qwe", 
				"mobile" => $phone, 
				"content" => "【、】尊敬的" . $mem['mem_name'] . "您的验证码是：" . $authnum . ",打死也不要告诉别人！", "sendTime" => "", "extno" => "");
...
/*
    function actionNote() {
        ...
        if (!empty($mem)) {
			...
			$postdata = array(
            "tempid" => "50044",
            "param" => urlencode("code=".$Num.""),
            "phone"=>$phone,
            "appkey" => "13915",
            "sign" => "f629571eaebfffbf4f7d552c9b4adae0",
            "format"=>"json",    );
             $url = "http://api.k780.com:88/?app=sms.send";
    ...
    function actionSendemail() {
        ...
        if (!empty($mem)) {
            if (Yii::app()->request->isAjaxRequest) {//是否ajax请求
                ...
                $mailer->Host = 'smtp.chaoyouwan.com';  //smtp服务器的名称（这里以163邮箱为例）
                ...
                $mailer->Subject = Yii::t('、玩友，请绑定邮箱', '注册码!');  //邮件主题
