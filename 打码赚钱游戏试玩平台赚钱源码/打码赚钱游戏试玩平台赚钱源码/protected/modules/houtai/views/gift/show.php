    ...
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#addBtn").fancybox({
                    'href': '<?php echo SITE_URL ?>houtai/gift/add',
                    'width': 733,
                    'height': 800,
                    'type': 'iframe',
                    ...
                    'onClosed': function() {
                        window.location.href = '<?php echo SITE_URL ?>houtai/gift/show';
                    }
                });
                /**编辑   **/
                $("a.edit").fancybox({
            ...
            function search(name) {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/gift/show").submit();
            }

            /** 新增**/
            function add() {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/gift/add").submit();
            }
            ...
            function del(fyID) {
                if (fyID == '')
                    return;
                if (confirm("您确定要删除吗？")) {
                    $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/gift/del/id/" + fyID).submit();
                }
            }
            ...
            function deleteSelected() {
                ...
                if (arr.length > 0) {
                    if (confirm("确认删除选中吗？")) {
                        $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/gift/alldel/ids/" + arr.substr(0, arr.length - 1)).submit();
                    }
                } else {
            ...
            <div class="ui_content">
                <div class="ui_text_indent">
                    <div id="box_border">
                        ...
                        <div id="box_center">
                            奖品名称&nbsp;&nbsp;<input type="text" id="name" name="name" class="ui_input_txt02" value="<?php echo $pages->params['name']; ?>"/>
                        </div>
                        <div id="box_bottom">
                    ...
                    <table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0" id="dataTab">
                        ...
                        foreach ($posts as $model) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php echo "<img src=/uploads/img/gift/" . $model['img'] . " style='width: 40px;height: 30px;'/>"; ?>
                                </td>
                                <td><?php echo $model['create_time']; ?></td>
                                <td><?php echo $model['name']; ?></td>
                                <td><?php echo $model['hld_num']; ?></td>
                                <td><?php
                                    $gifttype_info = Gifttype::model()->findByPk($model['gift_type_id']);
                                    echo $gifttype_info['name'];
                                    ?>
                                </td>
                                <td><?php
                                    if ($model['is_hpage'] == 1) {
                                ...
                                <td>
                                    <a href="<?php echo SITE_URL ?>houtai/gift/edit/id/<?php echo $model['id']; ?>" class="edit">编辑</a> 
