    ...
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#addBtn").fancybox({
                    'href': '<?php echo SITE_URL ?>houtai/message/add',
                    'width': 733,
                    'height': 600,
                    'type': 'iframe',
                    ...
                    'onClosed': function() {
                        window.location.href = '<?php echo SITE_URL ?>houtai/message/show';
                    }
                });
                /**编辑   **/
                $("a.edit").fancybox({
            ...
            function search(name) {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/message/show").submit();
            }

            /** 新增**/
            function add() {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/message/add").submit();
            }
            ...
            function del(fyID) {
                if (fyID == '')
                    return;
                if (confirm("您确定要删除吗？")) {
                    $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/message/del/id/" + fyID).submit();
                }
            }
            ...
            function deleteSelected() {
                ...
                if (arr.length > 0) {
                    if (confirm("确认删除选中吗？")) {
                        $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/message/alldel/ids/" + arr.substr(0, arr.length - 1)).submit();
                    }
                } else {
            ...
            <div class="ui_content">
                <div class="ui_text_indent">
                    <div id="box_border">
                        ...
                        <div id="box_center">
                            标题&nbsp;&nbsp;<input type="text" id="title" name="title" class="ui_input_txt02" value="<?php echo $pages->params['title']; ?>"/>
                        </div>
                        <div id="box_bottom">
                    ...
                    <table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0" id="dataTab">
                        ...
                        foreach ($posts as $model) {
                            ...
                            <tr>
                                ...
                                <td><?php echo $model['shortcont']; ?></td>
                                <td><?php
                                    $messagetype_info = Messagetype::model()->findByPk($model['message_type_id']);
                                    echo $messagetype_info['name'];
                                    ?>
                                </td>
                                <td><?php
                                    if ($model['is_hpage'] == 1) {
                                ...
                                <td><?php
                                    ...
                                    ?></td>
                                <td>
                                    <a href="<?php echo SITE_URL ?>houtai/message/edit/id/<?php echo $model['id']; ?>" class="edit">编辑</a> 
