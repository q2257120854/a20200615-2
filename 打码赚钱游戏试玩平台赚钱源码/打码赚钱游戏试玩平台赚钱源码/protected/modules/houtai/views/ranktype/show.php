    ...
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#addBtn").fancybox({
                    'href': '<?php echo SITE_URL ?>houtai/ranktype/add',
                    'width': 733,
                    'height': 230,
                    'type': 'iframe',
                    ...
                    'onClosed': function() {
                        window.location.href = '<?php echo SITE_URL ?>houtai/ranktype/show';
                    }
                });
                /**编辑   **/
                $("a.edit").fancybox({
            ...
            function search(name) {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/ranktype/show").submit();
            }

            /** 新增**/
            function add() {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/ranktype/add").submit();
            }
            ...
            function del(fyID) {
                if (fyID == '')
                    return;
                if (confirm("您确定要删除吗？")) {
                    $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/ranktype/del/id/" + fyID).submit();
                }
            }
            ...
            function deleteSelected() {
                ...
                if (arr.length > 0) {
                    if (confirm("确认删除选中吗？")) {
                        $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/ranktype/alldel/ids/" + arr.substr(0, arr.length - 1)).submit();
                    }
                } else {
            ...
            <div class="ui_content">
                <div class="ui_text_indent">
                    <div id="box_border">
                        ...
                        <div id="box_center">
                            类型名称&nbsp;&nbsp;<input type="text" id="name" name="name" class="ui_input_txt02" value="<?php echo $pages->params['name']; ?>"/>
                        </div>
                        <div id="box_bottom">
                    ...
                    <table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0" id="dataTab">
                        ...
                        foreach ($posts as $model) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="<?php echo SITE_URL ?>houtai/rank/show/typeid/<?php echo $model['id']; ?>" >排名管理</a> 
                                    <a href="<?php echo SITE_URL ?>houtai/ranktype/edit/id/<?php echo $model['id']; ?>" class="edit">编辑</a> 
