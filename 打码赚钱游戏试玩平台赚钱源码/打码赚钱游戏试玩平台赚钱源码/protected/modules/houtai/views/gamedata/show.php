    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function search(name) {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/gamedata/show").submit();
            }
            /** 删除 **/
            function del(fyID) {
                if (fyID == '')
                    return;
                if (confirm("您确定要删除吗？")) {
                    $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/gamedata/del/id/" + fyID).submit();
                }
            }
            ...
            function deleteSelected() {
                ...
                if (arr.length > 0) {
                    if (confirm("确认删除选中吗？")) {
                        $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/gamedata/alldel/ids/" + arr.substr(0, arr.length - 1)).submit();
                    }
                } else {
            ...
            //更新昨日
            function rank() {
                if (confirm("您确定要更新昨日的试玩排行么？")) {
                    $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/gamedata/rank").submit();
                }
            }
                    ...
                    <table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0" id="dataTab">
                        ...
                        foreach ($posts as $model) {
                            ...
                            <tr>
                                ...
                                <td><?php echo $mem_model->findByPk($model['mem_id'])->mem_name; ?></td>
                                <td><?php echo $model['username']; ?></td>
                                <td><?php echo $game_model->findByPk($model['game_id'])->name; ?></td>
