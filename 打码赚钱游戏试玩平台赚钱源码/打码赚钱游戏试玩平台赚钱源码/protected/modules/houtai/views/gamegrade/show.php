    ...
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#addBtn").fancybox({
                    'href': '<?php echo SITE_URL ?>houtai/gamegrade/add/gameid/<?php echo $pages->params['gameid']; ?>',
                                'width': 733,
                                'height': 430,
                                'type': 'iframe',
                                ...
                                'onClosed': function() {
                                    window.location.href = '<?php echo SITE_URL ?>houtai/gamegrade/show/gameid/<?php echo $pages->params['gameid']; ?>';
                                                }
                                            });
                                            /**编辑   **/
                                            $("a.edit").fancybox({
                                                    ...
                                                    window.location.href = '<?php echo SITE_URL ?>houtai/gamegrade/show/gameid/<?php echo $pages->params['gameid']; ?>';
                                                        ...
                                                        function del(fyID) {
                                                            if (fyID == '')
                                                                return;
                                                            if (confirm("您确定要删除吗？")) {
                                                                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/gamegrade/del/gameid/<?php echo $pages->params['gameid']; ?>/id/" + fyID).submit();
                                                            }
                                                        }
                                                        ...
                                                        function deleteSelected() {
                                                            ...
                                                            if (arr.length > 0) {
                                                                if (confirm("确认删除选中吗？")) {
                                                                    $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/gamegrade/alldel/gameid/<?php echo $pages->params['gameid']; ?>/ids/" + arr.substr(0, arr.length - 1)).submit();
                                                                }
                                                            } else {
    ...
    <body>
        ...
        <div id="container">
            ...
            <div class="ui_content">
                    ...
                    <table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0" id="dataTab">
                        ...
                        foreach ($posts as $model) {
                            ...
                            <tr>
                                ...
                                <td><?php echo $model['money']."元"; ?></td>
                                <td>
                                    <a href="<?php echo SITE_URL ?>houtai/gamegrade/edit/id/<?php echo $model['id']; ?>" class="edit">编辑</a> 
                ...
                <div>、网络奖励元宝总计：
                <?php 
                $gamehlb_sum=Gamegrade::model()->countBySql("select sum(hlb) from {{game_grade}} where game_id=".$gameid);
                  echo $gamehlb_sum;
                ?>
                </div>
                <div>厂商奖励金额总计：
                <?php $gamemoney_sum=Gamegrade::model()->countBySql("select sum(money) from {{game_grade}} where game_id=".$gameid);
                  echo $gamemoney_sum."元";
                ?>
                </div>
