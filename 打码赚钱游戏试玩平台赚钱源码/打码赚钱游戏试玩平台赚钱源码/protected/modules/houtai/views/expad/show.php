    ...
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#addBtn").fancybox({
                    'href': '<?php echo SITE_URL ?>houtai/expad/add',
                    'width': 733,
                    'height': 600,
                    'type': 'iframe',
                    ...
                    'onClosed': function() {
                        window.location.href = '<?php echo SITE_URL ?>houtai/expad/show';
                    }
                });
                /**编辑   **/
                $("a.edit").fancybox({
            ...
            function search(name) {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/expad/show").submit();
            }

            /** 新增**/
            function add() {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/expad/add").submit();
            }
            ...
            function del(fyID) {
                if (fyID == '')
                    return;
                if (confirm("您确定要删除吗？")) {
                    $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/expad/del/id/" + fyID).submit();
                }
            }
            ...
            function deleteSelected() {
                ...
                if (arr.length > 0) {
                    if (confirm("确认删除选中吗？")) {
                        $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/expad/alldel/ids/" + arr.substr(0, arr.length - 1)).submit();
                    }
                } else {
                    ...
                    <table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0" id="dataTab">
                        ...
                        foreach ($posts as $model) {
                            ...
                            <tr>
                                ...
                                <td><?php echo "<img src=/uploads/img/expad/" . $model['img'] . " style='width: 40px;height: 30px;'/>"; ?></td>
                                <td><?php echo $model['id']; ?></td>
                                <td><?php echo $model['cid']; ?></td>
                                <td><?php echo $model['create_time']; ?></td>
                                <td><?php echo $model['name']; ?></td>
                                <td><?php echo $model['content']; ?></td>
                                <td><?php echo $model['explain']; ?></td>
                                <td><?php echo $model['rewards_hlb']; ?></td>
                                <td><?php
                                    if ($model['is_timely'] == 0) {
                                        echo "即";
                                    } else {
                                        echo "手";
                                    }
                                ...
                                <td>
                                    <a href="<?php echo SITE_URL ?>houtai/expadgrade/show/expadid/<?php echo $model['id']; ?>" >等级奖励</a> 
                                    <a href="<?php echo SITE_URL ?>houtai/expad/edit/id/<?php echo $model['id']; ?>" class="edit">编辑</a> 
