    ...
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#addBtn").fancybox({
                    'href': '<?php echo SITE_URL ?>houtai/captcha/add',
                    'width': 733,
                    'height': 800,
                    'type': 'iframe',
                    ...
                    'onClosed': function() {
                        window.location.href = '<?php echo SITE_URL ?>houtai/captcha/show';
                    }
                });
                /**编辑   **/
                $("a.edit").fancybox({
            ...
            function search(name) {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/captcha/show").submit();
            }

            /** 新增**/
            function add() {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/captcha/add").submit();
            }
            ...
            function del(fyID) {
                if (fyID == '')
                    return;
                if (confirm("您确定要删除吗？")) {
                    $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/captcha/del/id/" + fyID).submit();
                }
            }
            ...
            function deleteSelected() {
                ...
                if (arr.length > 0) {
                    if (confirm("确认删除选中吗？")) {
                        $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/captcha/alldel/ids/" + arr.substr(0, arr.length - 1)).submit();
                    }
                } else {
                    ...
                    <table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0" id="dataTab">
                        ...
                        foreach ($posts as $model) {
                            ...
                            <tr>
                                ...
                                <td><?php echo $model['ad_id']; ?></td>
                                <td><?php echo $model['name']; ?></td>
                                <td><?php echo $model['title2']; ?></td>
                                <td><?php echo $model['code_val']; ?></td>
                                <td><?php echo $model['jiesuan']; ?></td>
                                <td><?php
                                    if (empty($model['ip'])) {
                                        echo "不换ip";
                                    } else if ($model['ip'] == 1) {
                                ...
                                <td>
                                    <a href="<?php echo SITE_URL ?>houtai/captcha/edit/id/<?php echo $model['id']; ?>" class="edit">编辑</a> 
