    ...
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#addBtn").fancybox({
                    'href': '<?php echo SITE_URL ?>houtai/rank/add/typeid/<?php echo $typeid;?>',
                    'width': 733,
                    'height': 430,
                    'type': 'iframe',
                    ...
                    'onClosed': function() {
                        window.location.href = '<?php echo SITE_URL ?>houtai/rank/show/typeid/<?php echo $typeid;?>';
                    }
                });
                /**编辑   **/
                $("a.edit").fancybox({
            ...
            function search(name) {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/rank/show/typeid/<?php echo $typeid;?>").submit();
            }

            /** 删除 **/
            function del(typeid, fyID) {
                if (fyID == '')
                    return;
                if (confirm("您确定要删除吗？")) {
                    $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/rank/del/typeid/" + typeid + "/id/" + fyID).submit();
                }
            }
            ...
            function deleteSelected() {
                ...
                if (arr.length > 0) {
                    if (confirm("确认删除选中吗？")) {
                        $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/rank/alldel/typeid/" +<?php echo $typeid; ?> + "/ids/" + arr.substr(0, arr.length - 1)).submit();
                    }
                } else {
            ...
            <div class="ui_content">
                <div class="ui_text_indent">
                    <div id="box_border">
                        ...
                        <div id="box_center">
                            等级名称&nbsp;&nbsp;<input type="text" id="name" name="name" class="ui_input_txt02" value="<?php echo $pages->params['name']; ?>"/>
                        </div>
                        <div id="box_bottom">
                        ...
                        foreach ($posts as $model) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php
                                    if ($model['start'] == "已开启") {
                                        echo "<span style='color:green;'>已开启</span>";
                                    } else {
                                ...
                                <td>
                                    <a href="<?php echo SITE_URL ?>houtai/rank/start/typeid/<?php echo $model['rank_type']; ?>/id/<?php echo $model['id']; ?>"  onclick="return confirm('确定要启用或者禁用吗？')">
                                        <?php
                                        if ($model['start'] == "已开启") {
                                    ...
                                    <a href="<?php echo SITE_URL ?>houtai/rank/edit/typeid/<?php echo   $model['rank_type']; ?>/id/<?php echo $model['id']; ?>" class="edit">编辑</a> 
                                    <a href="javascript:del('<?php echo $model['rank_type']; ?>','<?php echo $model['id']; ?>');">删除</a>
                                </td>
                            </tr>
                            <?php
                        }
