    ...
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#addBtn").fancybox({
                    'href': '<?php echo SITE_URL ?>houtai/adtype/add',
                    'width': 733,
                    'height': 230,
                    'type': 'iframe',
                    ...
                    'onClosed': function() {
                        window.location.href = '<?php echo SITE_URL ?>houtai/adtype/show';
                    }
                });
                /**编辑   **/
                $("a.edit").fancybox({
            ...
            function search(name) {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/adtype/show").submit();
            }

            /** 新增**/
            function add() {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/adtype/add").submit();
            }
            ...
            function del(fyID) {
                if (fyID == '')
                    return;
                if (confirm("您确定要删除吗？")) {
                    $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/adtype/del/id/" + fyID).submit();
                }
            }
            ...
            function deleteSelected() {
                ...
                if (arr.length > 0) {
                    if (confirm("确认删除选中吗？")) {
                        $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/adtype/alldel/ids/" + arr.substr(0, arr.length - 1)).submit();
                    }
                } else {
                    ...
                    <table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0" id="dataTab">
                        ...
                        foreach ($posts as $model) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="<?php echo SITE_URL ?>houtai/adtype/edit/id/<?php echo $model['id']; ?>" class="edit">编辑</a> 
