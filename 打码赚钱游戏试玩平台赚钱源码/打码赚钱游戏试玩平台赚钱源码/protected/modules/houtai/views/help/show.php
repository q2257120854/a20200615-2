    ...
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#addBtn").fancybox({
                    'href': '<?php echo SITE_URL ?>houtai/help/add',
                    'width': 733,
                    'height': 600,
                    'type': 'iframe',
                    ...
                    'onClosed': function() {
                        window.location.href = '<?php echo SITE_URL ?>houtai/help/show';
                    }
                });
                /**编辑   **/
                $("a.edit").fancybox({
            ...
            function search(name) {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/help/show").submit();
            }

            /** 新增**/
            function add() {
                $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/help/add").submit();
            }
            ...
            function del(fyID) {
                if (fyID == '')
                    return;
                if (confirm("您确定要删除吗？")) {
                    $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/help/del/id/" + fyID).submit();
                }
            }
            ...
            function deleteSelected() {
                ...
                if (arr.length > 0) {
                    if (confirm("确认删除选中吗？")) {
                        $("#submitForm").attr("action", "<?php echo SITE_URL ?>houtai/help/alldel/ids/" + arr.substr(0, arr.length - 1)).submit();
                    }
                } else {
            ...
            <div class="ui_content">
                <div class="ui_text_indent">
                    <div id="box_border">
                        ...
                        <div id="box_center">
                            提问&nbsp;&nbsp;<input type="text" id="quiz" name="quiz" class="ui_input_txt02" value="<?php echo $pages->params['quiz']; ?>"/>
                        </div>
                        <div id="box_bottom">
                    ...
                    <table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0" id="dataTab">
                        ...
                        foreach ($posts as $model) {
                            ...
                            <tr>
                                ...
                                <td><?php echo $model['quiz']; ?></td>
                                <td><?php
                                    $helptype_info = Helptype::model()->findByPk($model['help_type_id']);
                                    echo $helptype_info['name'];
                                    ?>
                                </td>
                                <td>
                                    <a href="<?php echo SITE_URL ?>houtai/help/edit/id/<?php echo $model['id']; ?>" class="edit">编辑</a> 
