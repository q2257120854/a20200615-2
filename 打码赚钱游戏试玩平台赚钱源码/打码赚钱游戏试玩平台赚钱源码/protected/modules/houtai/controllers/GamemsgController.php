...
class GamemsgController extends Controller {
    ...
    function actionShow() {
        $sql = "SELECT * FROM {{returnmsg}} where 1=1";
        $sb = '';
        $name = null;
    ...
    function actionAlldel($ids) {
        $returnmsg_model = Returnmsg::model();
        $arr = explode(",", $ids);
        foreach ($arr as $id) {
            $returnmsg_info = $returnmsg_model->findByPk($id);
            if ($returnmsg_info->delete()) {
