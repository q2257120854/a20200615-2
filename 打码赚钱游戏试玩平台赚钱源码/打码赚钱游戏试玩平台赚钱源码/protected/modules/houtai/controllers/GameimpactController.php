...
class GameimpactController extends Controller {
    ...
    function actionShow() {
        $gameid = $_GET['gameid'];
        $sql = "SELECT * FROM {{game_impact}} where game_id=" . $gameid . " order by id desc";
    ...
    function actionAdd() {
        $gameid = $_GET['gameid'];
        $impact_model = new Gameimpact();
        if (isset($_POST['Gameimpact'])) {
            foreach ($_POST['Gameimpact'] as $_k => $_v) {
                $impact_model->$_k = strip_tags($_v);
            }
            if ($impact_model->save()) {
                Yii::app()->user->setFlash('msg', '添加成功');
                $result = "success";
            }
        }
        $this->renderPartial('add', array('impact_model' => $impact_model, 'result' => $result, "gameid" => $gameid));
    }
    ...
    function actionEdit($id) {
        ...
        $this->renderPartial('edit', array('impact_model' => $impact_info, 'result' => $result));
    }
