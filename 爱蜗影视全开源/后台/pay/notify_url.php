...
$sql="select * from ap_user where id =$d[0] limit 1 ";
 ...
 while ($row = mysqli_fetch_assoc($result))
  { 
   ...
   $shanghu = array();
  $sql="select * from ap_shezi where id ={$a['id']} ";
   $result=mysqli_query($link,$sql);
 while ($row = mysqli_fetch_assoc($result))
  { 
	$shanghu = $row; 
  ...
  if(!$shanghu){
	  $sql="select * from ap_shezi where id ={$a['parentid']} ";
	   $result=mysqli_query($link,$sql);
	 while ($row = mysqli_fetch_assoc($result))
	  { 
		$shanghu = $row; 
...
if($verify_result) {//验证成功
  ...
  //$sendUrl = 'http://awys.com/login/login/pay.html';
  $sendUrl = 'http://awys.com/index/pay/createKm.html';
   ...
   $d = explode('|',$d[1]);
	...
	if($content){
		...
		if($error_code == 1){
			$msg = $result['msg'];
			echo "<script>layer.open({content:'$msg',btn: '确定',shadeClose: false,yes: function(){layer.open({content: '返回个人中心即可。',time: 2,skin:'msg'}); }});</script>";
			//echo '充值成功';
		}else{
