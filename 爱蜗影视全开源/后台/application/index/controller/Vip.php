...
*/
						...
						foreach($payInfo as $k=>$v){ 
							$type = db('pay_type')->where(['str_name'=>$v,'uid'=>session('usershshefsdf')])->find();
							if($v == 'daili'){
						    $url = trim('http://awys.com/dailipay/?name='.$type['name'].'&type_id='.$type['id'].'&fee='.$typeInfo1[$v]); 
							}else{
							$url = trim('http://awys.com/pay/?name='.$type['name'].'&type_id='.$type['id'].'&fee='.$typeInfo1[$v]); 	
							}
