...
if($verify_result || 1) {//验证成功
  ...
  $d = explode(':',$d[0]);
  $d = explode('|',$d[1]);
  $sendUrl = 'http://awys.com/index/pay/createmoney.html';
	...
	if($content){
		...
		if($error_code == 1){
			$msg = $result['msg'];
			echo "<script>layer.open({content:'操作成功',btn: '确定',shadeClose: false,yes: function(){layer.open({content: '返回个人中心即可。',time: 2,skin:'msg'}); }});</script>";
