...
if($verify_result || 1) {//验证成功
	...
	//$sendUrl = 'http://awys.com/login/login/dailipay.html';
  $d = explode('--',$_GET['name']);
  $name = $d[1];
  $d = explode(':',$d[0]);
  $d = explode('|',$d[1]);
  $sendUrl = 'http://awys.com/index/pay/createKm.html';
	...
	if($content){
		...
		if($error_code == 1){
			$msg = "<font color=red size=5>".$result['msg']."</font>";
			 echo "<script>layer.open({content:'".$msg."',btn: '<font color=green size=5>'+'如提示未开通请重新登录'+'</font>',shadeClose: false,yes: function(){layer.open({content: '如提示未开通请重新登录',time: 2,skin:'msg'}); }});</script>";
		}else{
			echo "失败【{$result['msg']}】";
		}
	}else{
