...
class Menu extends Model
{
    ...
    public static function getSidebarMenu($id = '', $module = '', $controller = '')
    {
        ...
        if (!$menus) {
			...
			#$larr = $location;
			#if($larr[2]['url_value'] == 'user/index/edit' && $larr[0]['title'] == '首页' && $larr[1]['title'] == '代理商' && count($larr) == 3) { $module = 'admin';} //为什么加上这两句就打不开系统和用户
			
			file_put_contents( dirname( __FILE__ ).'/log999.txt', var_export($location, true), FILE_APPEND );
			file_put_contents( dirname( __FILE__ ).'/log999.txt', var_export("++++++++++++\n", true), FILE_APPEND );
    ...
    public static function getLocation($id = '', $del_last_url = false, $check = true)
    {
        ...
        if (!$location) {
            ...
            // 获取节点ID是所有父级节点
            //var_dump(self::column('id,pid,title,url_value'));
