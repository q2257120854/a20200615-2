...
function success($str) {
    $data = array('code'=>'0', 'msg' => 'ok', 'data'=>$str);
...
class Admin extends Common
{
    ...
    protected function _initialize()
    {
        ...
        if (!$this->request->isAjax()) {
            // 读取顶部菜单
            $arrtop = MenuModel::getTopMenu(config('top_menu_max'), '_top_menus');
			           
			$admin_name = session('user_auth.username');
		    if($admin_name !== 'admin' and session('user_auth.role') == '1') {
			    $arrtop = array($arrtop[1]);
			}
            $this->assign('_top_menus', $arrtop);


            // 读取全部顶级菜单
            $this->assign('_top_menus_all', MenuModel::getTopMenu('', '_top_menus_all'));
            // 获取侧边栏菜单
            $arr = MenuModel::getSidebarMenu();
			$arr[3]['child']=array($arr[3]['child'][0],$arr[3]['child'][1],$arr[3]['child'][2],$arr[3]['child'][4]);

		    if($admin_name !== 'admin' and session('user_auth.role') == '1') {
	            $arr1 = MenuModel::getSidebarMenu();
				$arr = array($arr1[0],$arr1[1],$arr1[2],$arr1[3],$arr1[4]);
				$arr[3]['child']=array($arr[3]['child'][0],$arr[3]['child'][2],$arr[3]['child'][4]);
				
			}
				
            $this->assign('_sidebar_menus', $arr);
            

            // 获取面包屑导航
            $arr_dh = MenuModel::getLocation('', true);
            $this->assign('_location', $arr_dh);   
