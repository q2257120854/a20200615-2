...
require_once (dirname(__FILE__)."/llpay_core.function.php");
require_once (dirname(__FILE__)."/llpay_md5.function.php");
require_once (dirname(__FILE__)."/llpay_rsa.function.php");
use think\Log;
class LLpayNotify {
	var $llpay_config;
	...
	function verifyNotify() {
		//生成签名结果
			$is_notify = true;
			include_once (dirname(__FILE__).'/llpay_cls_json.php');
			$json = new JSON;
	...
	function getSignVeryfy($para_temp, $sign) {
		...
		Log::log("log.txt", "原串:" . $prestr . "\n" );
		Log::log("log.txt", "sign:" . $sign . "\n" );
		$isSgin = false;
