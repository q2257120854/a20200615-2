...
function getkuaishouid(){
	...
	if(kuauishouurl.indexOf('http')<0){layer.alert('请输入正确的KF作品链接！');return false;}
	...
	try{
		...
		layer.alert('请输入正确的KF作品链接！');return false;
	}
...
function get_kuaishou(id,ksid){
	if(ksid==''){
		...
		if(ksid==''){
			layer.alert('请先填写KF作品链接！');return false;
		}
	}
	var zpid = $('#'+id).val();
...
function getbilibiliid(){
	var songurl=$("#inputvalue").val();
	if(songurl==''){layer.alert('请确保每项不能为空！');return false;}
	if(songurl.indexOf('bilibili.com')< 0 || songurl.indexOf('b23.tv')){layer.alert('请输入正确的视频链接！');return false;}
	try{
...
function getCommentList(id,aweme_id,km,page){
	...
	page = page || 1;
	if(aweme_id==''){
		layer.alert('请先填写音乐作品ID！');return false;
	}
	if(aweme_id.length != 19){
		layer.alert('音乐作品ID填写错误');return false;
	}
	var ii = layer.load(2, {shade:[0.1,'#fff']});
	$.ajax({
		type : "GET",
