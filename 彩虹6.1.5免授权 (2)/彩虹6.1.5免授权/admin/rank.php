...
<div class="col-xs-12 col-md-10 center-block" style="float: none;">
    ...
    $cron_lasttime = $conf['cron_rank_time'];
    $cron_money    = $conf['cron_rank_money'];
    ?>
    <div class="block">
        <div class="block-title"><h3 class="panel-title">分站排行奖励设置</h3>&nbsp;&nbsp;[<a href="rank.php">返回</a>]</div>
        <div class="">
            <div class="alert alert-success">监控地址：<br/><a style="color:white" target="_blank"
                                                          href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/cron.php?do=rank&key=<?php echo $conf['cronkey']; ?>">http://<?php echo $_SERVER['HTTP_HOST'] ?>
                    /cron.php?do=rank&key=<?php echo $conf['cronkey']; ?></a></div>
            <div class="alert alert-warning">监控说明：每天一次或1小时一次都可以，只能在一个地方监控，千万不要多节点监控或在多处监控，否则会导致数据错乱。也可以每天凌晨手动打开一次。</div>
            <form action="./rank.php?my=setdo" method="post" class="form-horizontal" role="form"><input type="hidden"
                                                                                                        name="do"
                                                                                                        value="submit"/>
                <div class="form-group">
                    <label class="col-sm-2 control-label">每日奖励前几名</label>
                    <div class="col-sm-10"><input type="text" name="rank_reward"
                                                  value="<?php echo $conf['rank_reward']; ?>" class="form-control"
                                                  placeholder="（0或留空为关闭奖励）"/></div>
                </div>
                <br/>
                <div class="form-group">
                    <label class="col-sm-2 control-label">奖励销量的比例<br/>（百分数）</label>
                    <div class="col-sm-10"><input type="text" name="rank_percentage"
                                                  value="<?php echo $conf['rank_percentage']; ?>" class="form-control"
                                                  placeholder="（填写百分数，例如填写3将额外奖励当日销量的3%）"/></div>
                </div>
                <br/>
                <div class="form-group">
        ...
        <?php
            ...
            <div class="block">
                ...
                <ul class="nav nav-tabs">
                    <li class="<?php echo $_GET['last'] != 1 ? 'active' : null; ?>" style="width:33.333333%"><a
                                href="rank.php">
                    ...
                    <li style="width:33.333333%"><a href="rank.php?my=set">
                            <center>分站排行奖励设置</center>
                        </a></li>
                </ul>
                ...
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        foreach ($rs as $res) {
                            ...
                            echo '<tr><td class="text-center"><span class="badge badge-danger">' . $i . '</span></td><td class="text-center"><b>' . $res['zid'] . '</b></td><td class="text-center">' . mb_substr($res['sitename'], 0, 26, 'utf-8') . (mb_strlen($res['sitename']) > 26 ? '...' : '') . '</td><td class="text-center">' . $res['count'] . '</td><td class="text-center">' . $res['money'] . '</td>';
                            if ($conf['rank_reward'] > 0) {
                                if ($i <= $conf['rank_reward'])
                                    $reward = round($res['money'] * $conf['rank_percentage'] / 100, 2);
                                else
                                    $reward = 0;
