...
<div class="row">
    <div class="col-sm-12 col-md-10 center-block" style="float: none;">
        ...
        {
            global $DB, $conf;
            $rows      = $DB->get('workorder', '*', ['id' => $id]);
            $siterow   = $DB->get('site', ['zid', 'user', 'qq'], ['zid' => $rows['zid']]);
        ...
        $siterow  = $DB->get('site', ['zid', 'qq'], ['zid' => $rows['zid']]);
        ...
        <div class="panel panel-default">
            <div class="panel-heading"><h3 class="panel-title"><i
                            class="fa fa-sticky-note-o"></i>&nbsp;&nbsp;<b>工单详情</b></h3></div>
            ...
            <div class="gdan_gout">
                ...
                <div class="gdan_zhugan" style="border: none;">
                    <a href="./sitelist.php?zid=<?php echo $rows['zid'] ?>" target="_blank"><img
                                src="<?php echo $myimg ?>" class="img-circle" width="40"/></a>
                    <div class="gdan_kjia1">
                        ...
                        订单编号：<?php echo $rows['orderid'] ? '<a href="./orderList.php?id=' . $rows['orderid'] . '" target="_blank">' . $rows['orderid'] . '</a>' : '无订单号'; ?>
                        <br/>
                ...
                if ($rows['status'] == 2) {
                    ...
                    <div class="gdan_huifu">
                        <form action="./workorder-item.php?my=reply&id=<?php echo $id ?>" method="POST">
                            <textarea class="gdan_srk" name="content" placeholder="回复后工单状态自动变为已处理 ,分站站点将会收到通知哦！"
                                      required></textarea><br/>
                            <input type="checkbox" name="email" id="email" value="1" style="margin-left: 1%;"><label
                                    for="email">同时发送提醒邮件到用户邮箱</label><br/>
                            ...
                            <input type="button" name="submit" value="完结工单" class="gdan_huifu1"
                                   ...
                                   onclick="window.location.href='./workorder-item.php?my=complete&id=<?php echo $id ?>'"/>
                        </form>
                    </div>
                    <?php
                }
...
elseif ($my == 'reply') {
    ...
    if (empty($content)) {
        ...
        $updateResult = $DB->update('workorder', ['content' => $content, 'status' => 1], ['id' => $id, 'LIMIT' => 1]);
        if ($updateResult->rowCount() > 0) {
            if ($_POST['email'] == 1) send_msg_mail($id);
            exit("<script>alert('回复工单成功！');history.go(-1);</script>");
        } else {
    ...
    elseif ($rows['status'] == 2)
        showmsg('此工单已经结单', 3);
    $updateResult = $DB->update('workorder', ['status' => 2], ['id' => $id, 'LIMIT' => 1]);
    if ($updateResult->rowCount() > 0)
